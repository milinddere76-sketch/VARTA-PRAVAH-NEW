...
def fetch_news():
    ...
    # Simulation block for mock modes
    ...
    try:
        ...
        print(f"⚠️ Fetch error (Rate limit or network): {e}")
    # Hard fallback to prevent broadcast crashing
...
कृपया वरील बातमीसाठी 'Varta Pravah - {bulletin['name']}' या बुलेटिनची स्क्रिप्ट तयार करा:"""
    ...
    print(f"Generating Llama3 Script Sequence for: {bulletin['time_slot']}...")
...
# 4. Engine Generator (gTTS + FFmpeg Ticker Logic)
def generate_video(script, bulletin, breaking_text=None, is_female=False, is_promo=False):
    ...
    filter_complex = "[0:v]scale=1280:720[anchor]; [1:v][anchor]overlay=(W-w)/2:(H-h)/2[base];"
    filter_complex += "[2:v]scale=150:-1[logoscale]; [base][logoscale]overlay=W-w-50:50[base2];"
...
if __name__ == "__main__":
    ...
    detected_breaking_keyword = check_breaking_news(headlines["headline"])
    ...
    if detected_breaking_keyword:
        ...
        script = generate_script(headlines, urgent_bulletin)
        generate_video(script, urgent_bulletin, breaking_text=f"ब्रेकिंग न्यूज: {detected_breaking_keyword}")
    else:
        ...
        promo_script = generate_script({"headline": "Subscribe to VartaPravah", "description": "Keep it locked to VartaPravah for 24/7 non-stop ultra lit Marathi news streams!"}, promo_b)
